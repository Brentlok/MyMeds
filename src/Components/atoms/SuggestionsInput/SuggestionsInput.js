...
import {suggestItem} from 'src/actions/api_actions';
import {getByCode} from '../../../actions/api_actions';

const SuggestionsInput = forwardRef(
  ({border, number, password, autoComplete}, ref) => {
    ...
    const Suggestions = suggestions.map(item => (
      <TouchableOpacity
        ...
        key={item}
        style={stylesWithBorder.suggestion}>
    ...
    return (
      <>
        {showSuggestions && (
          <View style={stylesWithBorder.suggestionsBox}>{Suggestions}</View>
        )}
        <TextInput
          ref={inputRef}
